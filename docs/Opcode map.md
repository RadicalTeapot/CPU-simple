...
All additions are done with carry (A+B+Carry) to allow for easy multi-byte addition. **Do not forget to clear carry
before non-multi byte addition**.

Subtraction uses a no borrow carry (i.e. carry is set to 1 if no borrow occurred) to allow for easy multi-byte 
subtraction. **Do not forget to set carry before non-multi byte subtraction**.
