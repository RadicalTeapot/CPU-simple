namespace Emulator.Tests
{
    ...
    internal class Emulator_tests
    {
        ...
        {
            var code = Emulator.ParseArgs([], _logger, out var config, out _, out _);
            Assert.Multiple(() =>
            {
        ...
        {
            var code = Emulator.ParseArgs(["-m", "512"], _logger, out var config, out _, out _);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(0));
        ...
        {
            var code = Emulator.ParseArgs(["--memory", "1024"], _logger, out var config, out _, out _);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(0));
        ...
        {
            var code = Emulator.ParseArgs(["-s", "32"], _logger, out var config, out _, out _);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(0));
        ...
        {
            var code = Emulator.ParseArgs(["--registers", "8"], _logger, out var config, out _, out _);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(0));
        ...
        {
            var code = Emulator.ParseArgs(["-h"], _logger, out _, out _, out _);
        ...
        {
            var code = Emulator.ParseArgs(["--help"], _logger, out _, out _, out _);
            Assert.That(code, Is.EqualTo(1));
        }
        ...
        {
            var code = Emulator.ParseArgs(["--unknown"], _logger, out _, out _, out _);
        ...
        {
            var code = Emulator.ParseArgs(["-m", "abc"], _logger, out _, out _, out _);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(2));
        ...
        {
            var code = Emulator.ParseArgs(["-m"], _logger, out _, out _, out _);
            Assert.That(code, Is.EqualTo(2));
        }
        ...
        {
            var code = Emulator.ParseArgs(["-s", "notanumber"], _logger, out _, out _, out _);
            Assert.That(code, Is.EqualTo(2));
        }
        ...
        {
            var code = Emulator.ParseArgs(["--registers", "xyz"], _logger, out _, out _, out _);
            Assert.That(code, Is.EqualTo(2));
        }
        ...
        {
            var code = Emulator.ParseArgs(["-m", "512", "-s", "32", "--registers", "8"], _logger, out var config, out _, out _);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(0));
                Assert.That(config.MemorySize, Is.EqualTo(512));
        ...
        {
            var code = Emulator.ParseArgs([], _logger, out _, out int scale, out _);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(0));
        ...
        {
            var code = Emulator.ParseArgs(["--scale", "2"], _logger, out _, out int scale, out _);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(0));
        ...
        {
            var code = Emulator.ParseArgs(["--scale", "big"], _logger, out _, out _, out _);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(2));
                Assert.That(_logger.ErrorMessages, Has.Count.GreaterThan(0));
        ...
        {
            var code = Emulator.ParseArgs(["--scale"], _logger, out _, out _, out _);
            Assert.That(code, Is.EqualTo(2));
        }
        ...
        {
            var code = Emulator.ParseArgs(["--chr", "/some/file.chr"], _logger, out _, out _, out string? chrPath);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(0));
                Assert.That(chrPath, Is.EqualTo("/some/file.chr"));
            });
        }
        ...
        {
            var code = Emulator.ParseArgs(["--chr"], _logger, out _, out _, out _);
            Assert.That(code, Is.EqualTo(2));
        }
        ...
        {
            var code = Emulator.ParseArgs([], _logger, out _, out _, out string? chrPath);
            Assert.Multiple(() =>
            {
                Assert.That(code, Is.EqualTo(0));
                Assert.That(chrPath, Is.Null);
            });
        }
    }
}
