...
{
    [Opcode(OpcodeBaseCode.SBA, OpcodeGroupBaseCode.SingleRegisterALU)]
    internal class SBA : BaseOpcode
    {
        public SBA(byte instructionByte, State state, Memory memory, Stack stack)
        {
            _state = state;
            _memory = memory;
        ...
        private MicroPhase AluOp()
        {
            ...
            var result = registerValue - _addressValue - (1 - _state.GetCarryFlagAsInt());
