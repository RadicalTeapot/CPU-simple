-- Emulator process manager for cpu-simple
-- Handles async communication with Emulator.exe via STDIN/STDOUT/STDERR
...
function M.start(config)
  if M.running then
    vim.notify("Emulator is already running", vim.log.levels.WARN)
    return false
  end
  ...
  M.handle, M.pid = uv.spawn(M.config.emulator_path, spawn_opts, function(code, signal)
    M.on_exit(code, signal)
  end)

  if not M.handle then
    vim.schedule(function()
      vim.notify("Failed to start emulator: " .. tostring(M.pid), vim.log.levels.ERROR)
    end)
    M.cleanup()
    return false
  end
  ...
  M.stdout:read_start(function(err, data)
    if err then
      vim.schedule(function()
        vim.notify("Emulator stdout error: " .. err, vim.log.levels.ERROR)
      end)
      return
    end
    if data then
  ...
  -- Read stderr (logging/errors from emulator)
  M.stderr:read_start(function(err, data)
    if err then
      vim.schedule(function()
        vim.notify("Emulator stderr error: " .. err, vim.log.levels.ERROR)
      end)
      return
    end
    if data then
  ...
  vim.schedule(function()
    vim.notify("Emulator started (PID: " .. M.pid .. ")", vim.log.levels.INFO)
    events.emit(events.EMULATOR_STARTED, { pid = M.pid })
  end)

  return true
end

--- Send a command to the emulator via STDIN
---@param cmd string Command to send (without newline)
function M.send(cmd)
  if not M.running or not M.stdin then
    vim.notify("Emulator is not running", vim.log.levels.ERROR)
    return false
  end
...
function M.stop()
  if not M.running then
    vim.notify("Emulator is not running", vim.log.levels.WARN)
    return
  end
...
--- Handle stdout data from emulator
...
--- Handle stderr data from emulator (logs/errors)
---@param data string Data received
function M.on_stderr(data)
  vim.schedule(function()
    ...
    for _, line in ipairs(lines) do
      if line ~= "" then
        -- Parse log level from emulator format: [ERROR] only for now
...
function M.on_exit(code, signal)
  vim.schedule(function()
    ...
    M.cleanup()
    state.clear()
    events.emit(events.EMULATOR_STOPPED, { code = code, signal = signal })

    if code == 0 then
      vim.notify("Emulator exited", vim.log.levels.INFO)
    else
      vim.notify("Emulator exited with code " .. code .. " (signal: " .. signal .. ")", vim.log.levels.WARN)
    end
  end)
end
