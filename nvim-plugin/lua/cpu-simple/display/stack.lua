-- Stack panel for cpu-simple
-- Displays stack contents as a hex dump
...
local PANEL_ID = "stack"
local BUFFER_NAME = "[CPU-Simple-Stack]"
...
--- Show the stack panel in the sidebar
...
--- Render stack data into the buffer
function M.render()
  ...
  if state.stack and #state.stack > 0 then
    lines = format_hex_dump(state.stack, M.bytes_per_line)
  else
    table.insert(lines, "No stack data available")
  end
  ...
  -- Highlight the byte at SP
  if state.status and state.status.sp ~= nil and state.stack and state.status.sp < #state.stack then
    highlights.highlight_hex_dump_byte(bufnr, highlights.ns_stack_sp, highlights.groups.SP, state.status.sp, M.bytes_per_line)
  end
end
...
-- Subscribe to events
events.on(events.STACK_UPDATED, function()
...
events.on(events.STATUS_UPDATED, function()
  if M.is_visible() and state.stack then
    M.render()
  end
end)
