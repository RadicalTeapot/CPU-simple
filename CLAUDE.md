...
# Run the backend debugger (headless, 10Hz)
...
dotnet run --project Backend/Backend.csproj -- --vram 256 --chr /path/to/tiles.chr
...
- **Windowed vs headless mode**: `BackendApplication.Run()` dispatches to `RunWindowed()` when a `Display` was constructed (i.e., `--vram` was passed), otherwise `RunHeadless()`. In windowed mode: `TickFrame()` runs a full PPU frame's worth of state ticks per loop iteration; `IOutput.StatusSuppressed` is set around `TickFrame()` to suppress per-tick JSON flooding; `Display.Render()` throttles output to 60fps via Raylib's `SetTargetFPS`. `Display` is `internal` and only instantiated in `BackendApplication` — test code never creates it, so tests do not load Raylib natives.
...
- **MMIO bus decoder**: `BusDecoder` (in `CPU/components/BusDecoder.cs`) implements `IBus` and sits between the CPU and all addressable devices. Routes addresses `0x00–0xEF` to `Memory` and `0xF0–0xFF` to the `IMmioDevice` (PPU registers) in 8-bit mode; `0x0000–0xFEFF` to RAM and `0xFF00–0xFFFF` to MMIO in 16-bit mode. `IMmioDevice.ReadRegister`/`WriteRegister` receive byte offsets (base address already subtracted). When no PPU is present, an empty `MmioRouter` (no registered devices) is used. All bus accesses are recorded as `BusType.Memory` so existing `AddressWatchpoint` works on PPU register addresses. The stack is unaffected — stack operations bypass the address bus entirely.
