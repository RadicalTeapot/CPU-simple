...
- **Emulator tests**: `EmulatorTestHelpers.CreateGlobalContext()` creates a `GlobalCommandExecutionContext` with a real CPU and test doubles (`TestLogger`, `TestOutput`). Emulator's `ParseArgs` is `internal` for direct testability.
